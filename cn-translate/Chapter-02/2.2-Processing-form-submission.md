...
在程序清单 2.4 中，使用 @GetMapping 注释指定 `showDesignForm()` 方法应该处理 HTTP GET 请求 `/design`。与 @GetMapping 处理 GET 请求一样，可以使用 `@PostMapping` 处理 POST 请求。为了处理玉米卷艺术家提交的设计，将以下程序清单中的 `processTaco()` 方法添加到 DesignTacoController 中。
